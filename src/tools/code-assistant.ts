...
export const codeAssistantTools: Tool[] = [
    ...
    {
        name: 'code_plan',
        description: '为复杂的功能开发、架构变更或大规模重构创建详细的实施计划。包括需求分析、步骤分解、风险评估和测试策略。',
        inputSchema: {
            type: 'object',
            properties: {
                feature: {
                    type: 'string',
                    description: '需要规划的功能或任务描述',
                },
                context: {
                    type: 'string',
                    description: '现有系统或相关代码的背景信息(可选)',
                },
            },
            required: ['feature'],
        },
    ...
    {
        name: 'code_architect',
        description: '作为资深软件架构师，对系统设计、技术选型和架构决策提供专业咨询。生成架构决策记录(ADR)并评估权衡。',
        inputSchema: {
            type: 'object',
            properties: {
                decision: {
                    type: 'string',
                    description: '需要评估的架构决策或设计问题',
                },
                system_context: {
                    type: 'string',
                    description: '系统的当前架构和约束条件',
                },
            },
            required: ['decision'],
        },
    ...
    {
        name: 'code_tdd',
        description: '以测试驱动开发(TDD)专家的身份，指导开发流程。确保先写测试后写实现，并覆盖所有边界情况和异常路径。',
        inputSchema: {
            type: 'object',
            properties: {
                task: {
                    type: 'string',
                    description: '需要实现的开发任务',
                },
                language: {
                    type: 'string',
                    description: '编程语言',
...
请按照以下维度进行详细审查，并严格遵循 **Everything Claude Code** 的最佳实践：

1. **安全性 (Security) ⭐⭐⭐⭐⭐**：
   - [ ] **严禁硬编码**: 检查是否存在硬编码的密钥、API Token、密码等敏感信息。
   - [ ] **输入验证**: 验证所有外部输入是否经过了严格的校验和清洗。
   - [ ] **防御性编程**: 检查是否存在 SQL 注入、XSS、CSRF 等漏洞风险。
   - [ ] **错误处理**: 确保错误信息不会泄露系统敏感数据。

2. **代码质量与风格 (Coding Style) ⭐⭐⭐⭐⭐**：
   - [ ] **不可变性 (Immutability)**: 始终优先创建新对象，而不是修改现有对象。严禁副作用。
   - [ ] **文件限制**: 单个文件应小于 800 行（建议 200-400 行）。
   - [ ] **函数限制**: 单个函数应小于 50 行，保持单一职责。
   - [ ] **嵌套深度**: 嵌套层级严禁超过 4 层。
   - [ ] **命名规范**: 变量、函数、类名必须清晰且具有描述性。

3. **测试要求 (Testing) ⭐⭐⭐⭐**：
   - [ ] **测试覆盖率**: 核心逻辑必须保证 80% 以上的测试覆盖率。
   - [ ] **TDD 工作流**: 检查是否遵循“先写测试，再写实现”的红-绿-重构循环。
   - [ ] **测试类型**: 是否包含必要的单元测试、集成测试或 E2E 测试。

4. **性能与效率 (Performance) ⭐⭐⭐⭐**：
   - [ ] **复杂度分析**: 识别时间或空间复杂度过高的算法。
   - [ ] **瓶颈识别**: 发现潜在的 IO 阻塞、内存泄漏或重复计算。

5. **最佳实践与设计 (Architecture) ⭐⭐⭐**：
${getLanguageRules(language)}
   - [ ] **SOLID 原则**: 是否遵循单一职责、开闭原则等。
   - [ ] **低耦合**: 模块之间是否过度依赖。

6. **代码异味 (Code Smells) 识别**：
   - 过长的参数列表、过大的类、重复代码 (DRY)、魔法数字、不当的注释。
...
你是一位资深软件工程专家。请为以下需求创建一个详尽的实施计划：
...
请遵循 **Everything Claude Code** 的规划流程：
...
3. **详细实现步骤 (分阶段)**：
   - **Phase 1: 基础与后端** (明确文件路径、具体动作、原因、依赖、风险)。
   - **Phase 2: 核心功能** (增量式开发，每步可验证)。
   - **Phase 3: 边界处理与打磨** (异常路径、边缘情况)。

4. **测试策略**：
   - 明确单元测试、集成测试和 E2E 测试的重点文件和流程。

5. **风险与减缓**：
   - 识别潜在的技术债务或性能瓶颈及应对方案。

请输出结构清晰、极具可操作性的 Markdown 计划书。`,
                    },
                ],
            };
        }
        ...
        case 'code_architect': {
            const { decision, system_context } = args;
...
请输出一份标准的 **架构决策记录 (ADR)**，包含：

1. **Context (上下文)**：待解决的技术痛点或需求。
2. **Decision (决策)**：推荐的最终方案及核心理由。
3. **Consequences (后果)**：
   - **Positive (正面影响)**：收益、开发效率提升等。
   - **Negative (负面影响)**：引入的复杂度、成本、限制。
4. **Alternatives Considered (备选方案)**：对比其他候选方案，并说明为何不选。
5. **Architectural Principles (遵循原则)**：模块化、扩展性、安全性和性能考量。
...
你是一位 TDD 专家。请指导 \${language} 开发者按照“测试驱动”的方式实现以下任务：
...
1. **Write Test First (RED)**：
   - 编写一段描述预期行为的失败测试。
   - 涵盖：正常路径、Null/Empty 输入、边界值、错误路径。

2. **Write Minimal Implementation (GREEN)**：
   - 编写恰好能让测试通过的最少代码，严禁过度设计。

3. **Refactor (IMPROVE)**：
   - 在测试保证下，优化命名、消除重复、应用设计模式。
